...
class State(object):
    ...
    def getActions(self):
                        ...
                        if (nx,ny) not in self.actors and (nx,ny) not in v and self.grid[ny][nx] == '.':
                            ...
                            if (nx,ny) not in blocked:
                                if (nx,ny) not in room_cells and can_exit:
                                    # heappush(cur_actions, (new_cost, (a,(nx,ny))))
        ...
        # while len(cur_actions):
        #     yield heappop(cur_actions)
        return cur_actions
...
def bfs(init_state):
    ...
    while len(pq):
        ...
        if rnd % 5000 == 0:
            state.print()
        if state in v and v[state] < cost:
            continue
        if state.isComplete():
            return state.cost
        for action in state.getActions():
            ...
            if next_state not in v or v[next_state] > next_state.cost:
                ...
                heappush(pq, (next_state.cost,next_state))
    return -1
